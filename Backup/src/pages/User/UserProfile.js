...
const HealthcareStaffProfile = () => {
  const [profile, setProfile] = useState({
    name: 'Healthcare Staff Name',
    age: '30',
  ...
  useEffect(() => {
    const storedProfile = localStorage.getItem('healthcareStaffProfileData');
    if (storedProfile) {
  ...
  const handleSave = () => {
    ...
    setIsEditMode(false);
    localStorage.setItem('healthcareStaffProfileData', JSON.stringify(editableProfile));
  };
        ...
        <CardContent sx={{ padding: 2, paddingTop: 8 }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              {!isEditMode ? (
                <Typography variant="body2" sx={{ color: '#333' }}>{editableProfile.department}</Typography>
              ) : (
                <TextField
                  name="department"
...
export default HealthcareStaffProfile;
